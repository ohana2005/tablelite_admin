...
$(function(){
	...
	$('#menuDelFiles').click(function() {
		...
		if(files.length == 0) {
			...
			if(confirm('Selected files: '+files.length+'\n\nConfirm please...')) {
				$('#loader').show();
				var path = getCurrentPath();
	...
	function showFootInfo() {
		...
		if(file.length > 1) {
			...
			$('#footExt').text('Selected files: '+file.length);
			var tmpSizeCount = 0;
			...
			$('#footExt').text('File count: '+allFiles.length);
			var tmpSizeCount = 0;
    ...
    $('#fileOpen').MultiFile({ 
		STRING: { 
			...
			denied:'Extension $ext is not supported!',
			duplicate:'File $file is alredy exists'
		},
		max: 5,
		...
		$('#filesForm').ajaxSubmit({
			success: function(){
				$('#fileOpen_wrap_labels').slideUp(function(){
					...
					$('#fileOpen').MultiFile({ 
						STRING: {
							remove:'<img src="img/cross_small.png" width="16" height="16" alt="Cancel" />',
							denied:'Extension $ext is not supported!',
							duplicate:'File $file is alredy exists'
						},
						max: 5,
