(function($) {
  ...
  ColorPicker.prototype = {
    ...
    setColor: function(color) {
      ...
      this.$colorTarget.css('background-color', this._normalizeColor(color));
    ...
    setMode: function(mode) {
      ...
      if (mode !== 'flat') {
        var $button = null,
            ...
            func = function(e) {
              e.stopPropagation();
              if(_this.options.generateButton) { $buttonColor.css('background-color', _this._normalizeColor(_this.options.color)); }
              _this._toggleEffect();
            };

        if(this.options.generateButton) {
          $button = $('<div class="wColorPicker-button"></div>');
          $buttonColor = $('<div class="wColorPicker-button-color"></div>').css('background-color', this._normalizeColor(this.options.color));
    ...
    _createColor: function(index, color) {
      ...
      return $('<div class="wColorPicker-palette-color"></div>')
        ...
        .css('background-color', this._normalizeColor(color))
        .hover(
          function(){ _this._colorMouseenter($(this)); },
          function(){ _this._colorMouseleave($(this)); }
        )
        .click(function(e) {
          _this.setColor(window.rgbHex($(this).css('background-color')));
        });
    },
    ...
    _customInputKeyup: function(e) {
        ...
        if (window.rgbHex(val)) {
          this.$colorTarget.css('background-color', this._normalizeColor(val));
          if (e.keyCode === 13) { this.setColor(val); }
        }
    },

    _colorMouseenter: function($el) {
      var color = window.rgbHex($el.css('background-color'));
    ...
    _colorMouseleave: function($el) {
      ...
      this.$colorTarget.css('background-color', this._normalizeColor(this.options.color));
    ...
    _normalizeColor: function(color){
        if(color == 'transparent' || color.charAt(0) == '#'){
            return color;
        }
        return '#' + color;
    }
