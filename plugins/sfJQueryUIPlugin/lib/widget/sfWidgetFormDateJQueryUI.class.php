...
class sfWidgetFormDateJQueryUI extends sfWidgetForm
{
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
    ...
    $params = json_encode($this->getOption('params'));
...
EOHTML;
    if($minDate || $maxDate){
        $html .= <<<EOHTML
        if(!Date.parse){
            alert("You need to install date.js library to parse date correctly");
            return;
        }
        
EOHTML;
        if($minDate){
            $html .= "minDate = Date.parse('$minDate');";
        }
        if($maxDate){
            $html .= "maxDate = Date.parse('$maxDate');";
        }        
        
    }
    $html .= <<<EOHTML
    var params = $.extend($.datepicker.regional['$culture'], $params, {
    minDate: minDate,
    maxDate: maxDate,
    changeMonth : $cm,
    changeYear : $cy,
