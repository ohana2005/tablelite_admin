...
<div class="sf_admin_form">
  ...
  [?php echo form_tag_for($form, '@<?php echo $this->params['route_prefix'] ?>') ?]
    ...
    [?php foreach ($configuration->getFormFields($form, $form->isNew() ? 'new' : 'edit') as $fieldset => $fields): ?]
      [?php include_partial('<?php echo $this->getModuleName() ?>/form_fieldset', array('<?php echo $this->getSingularName() ?>' => $<?php echo $this->getSingularName() ?>, 'form' => $form, 'fields' => $fields, 'fieldset' => $fieldset, 'configuration' => $configuration)) ?]
    [?php endforeach; ?]
...
<script type='text/javascript'>
    ...
    $(function(){
       if($('.sf_admin_form fieldset').size() > 1){
           $('.sf_admin_form_tabs').append('<ul class="nav nav-tabs"></ul>');
           $('form fieldset').each(function(){
              var icon = $(this).find('h2').data('icon') ? ("<i class='fa fa-%icon%'></i> ").replace('%icon%', $(this).find('h2').data('icon')) : '';
              var $tab = $(("<li role='presentation' class='sf_admin_form_tab' rel='%rel%'><a href='#'>%icon%%name%</a></li>")
                .replace('%rel%', $(this).prop('id'))
                .replace('%name%', $(this).find('h2').text())
                .replace('%icon%', icon)
                )
                ;
              $('.sf_admin_form_tabs ul').append($tab);
              $(this).hide();
           });
           ...
           $('.sf_admin_form_tabs .sf_admin_form_tab').each(function(){
              $(this).click(function(){
                 ...
                 $('.sf_admin_form_tab').removeClass('active');
           ...
           if($.cookie(cookie_name)){
               ...
               $($('.sf_admin_form_tabs .sf_admin_form_tab').get(0)).click();
           }
       }else{
           $('.sf_admin_form_tabs').hide();
       }
    });
